...
//Función para obtener y filtrar datos de la ciudad desde una URL JSON
    ...
    https.get(jsonUrl, (response) => {
      ...
      response.on('end', () => {
        try {
          ...
          const filteredData = jsonData.map(({ id, ciudades }) => ({ id, ciudades }));
...
router.get('/', async (req, res) => {
  ...
  try {
    const filteredJsonData = await getDataCiudad(jsonUrl);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ getDataCiudad: filteredJsonData }));
  } catch (error) {
